...
public class UserController extends BaseController {
    ...
    @GetMapping("/{id}")
    public Result findById(@ApiParam("ID") @PathVariable Long id) {
        return toResp(userService.findById(id), "no such user.");
    }

    @ApiOperation("通过电话查询单个用户")
    @GetMapping("/phone/{phone}")
    public Result findByPhone(@ApiParam("电话") @PathVariable String phone) {
        return toResp(userService.findUserByPhone(phone), "no such user.");
    }

    @ApiOperation("通过邮箱查询单个用户")
    @GetMapping("/email/{email}")
    public Result findByEmail(@ApiParam("邮箱") @PathVariable String email) {
    ...
    public Result insert(@RequestBody User user) {
        if (UserConstants.NOT_UNIQUE.equals(userService.checkAccountUnique(user.getUserAccount()))) {
            return Result.error("新增用户'" + user.getUserAccount() + "'失败，登录账号已存在");
        } else if (UserConstants.NOT_UNIQUE.equals(userService.checkPhoneUnique(user))) {
            return Result.error("新增用户'" + user.getUserAccount() + "'失败，手机号码已存在");
        } else if (UserConstants.NOT_UNIQUE.equals(userService.checkEmailUnique(user))) {
            return Result.error("新增用户'" + user.getUserAccount() + "'失败，邮箱账号已存在");
        }
    ...
    @PostMapping("/login")
    public Result login(@RequestBody @Validated PhoneLoginDTO phoneLoginDTO) {
        User user = userService.login(phoneLoginDTO);
        if (user == null) {
            return Result.error("验证码错误或用户名不存在");
        }
